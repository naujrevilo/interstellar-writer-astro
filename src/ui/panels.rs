                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for collection in collections {
                            if ui
                                .selectable_label(
                                    selected_collection.as_ref() == Some(collection),
                                    collection,
                                )
                                .clicked()
                            {
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for entry in files {
                            ...
                            if ui
                                .selectable_label(
                                    selected_file.as_ref() == Some(&entry.name),
                                    label,
                                )
                                .clicked()
                            {
    ...
    egui::SidePanel::right("right_metadata")
        ...
        .show_animated(ctx, showing && has_file, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if components::secondary_button(ui, "🔧")
                        .on_hover_text("Recalcular rutas")
                        .clicked()
                    {
                        on_fix_paths();
                    }
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ui.with_layout(egui::Layout::top_down_justified(egui::Align::LEFT), |ui| {
                    if let Some(def) = collection_def {
                        ...
                        ui.group(|ui| {
                            ui.label(
                                egui::RichText::new("General")
                                    .strong()
                                    .size(tokens.font_size_md + 2.0),
                            );
                            ui.add_space(tokens.spacing_xs + 1.0);

                            for field_name in
                                &["title", "description", "date", "publishDate", "draft"]
                            {
                                if let Some(field) =
                                    def.fields.iter().find(|f| f.name == *field_name)
                                {
                        ...
                        ui.group(|ui| {
                            ui.label(
                                egui::RichText::new("Taxonomía e Imágenes")
                                    .strong()
                                    .size(tokens.font_size_md + 2.0),
                            );
                            ui.add_space(tokens.spacing_xs + 1.0);
                            ...
                            for field in &def.fields {
                                ...
                                if handled_keys.contains(&yaml_key) {
                                    continue;
                                }
                        ...
                        ui.group(|ui| {
                            ui.label(
                                egui::RichText::new("Componentes MDX")
                                    .strong()
                                    .size(tokens.font_size_md + 2.0),
                            );
                            ui.add_space(tokens.spacing_xs + 1.0);
                            ...
                            if components::primary_button(ui, "📦 Importar Notice")
                                ...
                                .clicked()
                            {
                                let rel_path = calculate_rel_path(
                                    config_content_dir,
                                    &coll,
                                    &file_name,
                    ...
                    if ui.button("🖼").clicked() {
                        if let Some(repo) = repo_path {
                            if let Some(path) = FileDialog::new()
                                ...
                                .pick_file()
                            {
                                if let Some(repo_rel_path) =
                                    crate::services::files::copy_image_to_assets(
                                        repo, assets_dir, &path,
                                    )
                                {
                                    *value = serde_yaml::Value::String(calculate_rel_path(
                                        content_dir,
                                        collection,
                                        file,
                                        &repo_rel_path,
                        ...
                        for (i, val) in seq.iter().enumerate() {
                            if let Some(tag) = val.as_str() {
                                let color = if field.field_type == FieldType::Categories {
                                    theme::tokens_from_ui(ui).brand_primary
                                } else {
                                    theme::tokens_from_ui(ui).text_muted
                                };
                                if components::status_badge(ui, &format!("{} ✖", tag), color)
                                    .clicked()
                                {
                                    to_remove = Some(i);
                                }
